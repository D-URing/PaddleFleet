#!/bin/bash
# Cascade refine v5: keep train_mode=attn_norm (avoid first-stage volatility).
# Upgrades vs v4:
#   1. target_mode=student_input  -> mitigate deep-layer distribution shift
#   2. train_window=4             -> joint-align 4 layers per endpoint
#   3. larger data (16384 samples) + more steps (4000) + stricter early-stop
set -euo pipefail
...
OUT=$ROOT/PaddleFleet/Research/VHA-Warmup/output/qwen3_vha_1p7B_refine_cascade_v5
LOG=$OUT/runlog

mkdir -p $OUT
cd $ROOT/PaddleFleet/Research/VHA-Warmup/scripts

VHA_INIT=$ROOT/PaddleFleet/Research/VHA-Warmup/output/qwen3_vha_1p7B_kv_postmix_activation_128
