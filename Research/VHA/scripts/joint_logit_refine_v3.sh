#!/bin/bash
# Joint logit refine v3: increase data + steps over v2 to reduce single-batch noise.
# Architecture: attn-only (FFN stays frozen, full unfreezing happens in warmup).
# Init checkpoint: cascade_v4 (16Q + postmix-only, correct architecture).
set -e
...
EXP_NAME=${EXP_NAME:-qwen3_vha_1p7B_joint_logit_v3}
...
# v3 vs v2: 4x data (4096 -> 16384), 2x steps (3000 -> 6000)
NUM_SAMPLES=${NUM_SAMPLES:-16384}
SEQ_LENGTH=${SEQ_LENGTH:-2048}
REFINE_STEPS=${REFINE_STEPS:-6000}
LR=${LR:-3e-5}
